...
class Member extends BaseApi
{
	...
	public function coupon()
	{
		...
		foreach ($counpon_list as $key => $item) {
			$counpon_list[ $key ]['start_time'] = date("Y-m-d H:i", $item['start_time']);
			$counpon_list[ $key ]['end_time'] = date("Y-m-d H:i", $item['end_time']);
		}
	...
	public function idCard(){
		$member = new MemberService();
		$idcard_info = $member->getIdCardInfo();
		$title = "身份证照片位置信息";
		return $this->outMessage($title, $idcard_info);
	}
	...
	public function pointExchangeBalance()
	{
		...
		if( $point == 200 || $point == 500 || $point = 1000){
			$point = (float) $point;
			$result = [];
			if ($point > 0) {
				$member = new MemberService();
			...
			return $this->outMessage($title, "", '-9999', "兑换积分额度有误");
		}
	...
	public function collection()
	{
		...
		foreach ($goods_collection_list['data'] as $k => $v) {
			if ($v['point_exchange_type'] == 0 || $v['point_exchange_type'] == 2) {
				$goods_collection_list['data'][ $k ]['display_price'] = $web_info['money_sign'] . $v["promotion_price"];
			} else {
				if ($v['point_exchange_type'] == 1 && $v["promotion_price"] > 0) {
					$goods_collection_list['data'][ $k ]['display_price'] = $web_info['money_sign']. $v["promotion_price"] . '+' . $v["point_exchange"] . '积分';
				} else {
	...
	public function shareContents()
	{
		...
		switch ($flag) {
			case "goods":
				...
				$share_content["share_contents"] = $config["goods_param_1"] . $web_info['money_sign'] . $goods_detail["price"] . ";" . $config["goods_param_2"];
			...
			case "bargain":
				...
				$share_content["share_title"] = '我正在' . $launch_info['bargain_min_money'] . $web_info['money_unit'].'拿' . $bargain_goods["goods_name"] . '就差你一刀了';
				$share_content["share_contents"] = '原价：' . $web_info['money_sign'] . $launch_info["goods_money"] . ";";
				$share_content['share_nick_name'] = $current_user;
				$album = new Album();
	...
	public function logOut()
	{
		...
		if (isInAlipayClient()) {
			Session::set('ali_autologin_lock', true);
		}
