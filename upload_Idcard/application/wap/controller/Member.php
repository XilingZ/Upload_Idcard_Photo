...
class Member extends BaseWap
{
	...
	{
		$this->assign('title', "身份证照片");
		$this->assign('title_before', "身份证照片");
		return $this->view($this->style . "member/idcard");
	}

	public function save(){
		$file_front = request()->file('image_front');
		$file_back = request()->file('image_back');
		$img_front = Db::table('sys_user')->where('uid',$this->uid)->value('idcard_front');
		$img_back = Db::table('sys_user')->where('uid',$this->uid)->value('idcard_back');
		if($file_front){
			if($img_front != null){
				unlink(__ROOT__ .'template/wap/default/public/img/id_card/'.$img_front);
			}
			$info = $file_front->move(__ROOT__ .'template/wap/default/public/img/id_card');
			$imgUrl = $info->getSaveName();
			$id_image = \think\Image::open(__ROOT__.'template/wap/default/public/img/id_card/'.$imgUrl);
			$id_image->thumb(500,500)->save(__ROOT__ . 'template/wap/default/public/img/id_card/'. $imgUrl);
			$res = Db::name('sys_user')->where('uid',$this->uid)->setField('idcard_front',$imgUrl);
		}
		if($file_back){
			if($img_back != null){
				unlink(__ROOT__ .'template/wap/default/public/img/id_card/'.$img_back);
			}
			$info = $file_back->move(__ROOT__ .'template/wap/default/public/img/id_card');
			$imgUrl = $info->getSaveName();
			$id_image = \think\Image::open(__ROOT__.'template/wap/default/public/img/id_card/'.$imgUrl);
			$id_image->thumb(500,500)->save(__ROOT__ . 'template/wap/default/public/img/id_card/'. $imgUrl);
			$res = Db::name('sys_user')->where('uid',$this->uid)->setField('idcard_back',$imgUrl);
		}
		return $this->view($this->style . "member/idcard");
	}
	...
	public function exchange()
	{
		...
		$count = $member_account->getTodayExchangeCount();
		$this->assign('exchanged', $count);
