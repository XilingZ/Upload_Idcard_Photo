...
/**
 * Member.php
 * IBO Mart 商城系统 - 全球电商解决方案
 * =========================================================
 * Copy right 2015-2025 ibomart.com 保留所有权利
 * ----------------------------------------------
 * 官方网址: http://ibomart.com
...
use data\extend\log\CLogFileHandler;
use data\extend\log\Log;
...
// 			//获取各项总值
	...
	public function memberdetail()
	{
		...
		//查询会员身份证照片
		$img_front = Db::table('sys_user')->where('uid',$member_id)->value('idcard_front');
		$img_back = Db::table('sys_user')->where('uid',$member_id)->value('idcard_back');
		$this->assign('idcard_front_url', $img_front);
		$this->assign('idcard_back_url', $img_back);
	...
	public function sysLogin()
	{
		...
		$uid = $member->sysLogin($uid);
		if ($uid > 0) {
			$member_info = $member->getMemberLoginInfoByUid($uid);
			$rtn = array(
				'code' => 0,
				'data' => $member_info 
			);
	...
	{
		$logHandler = new CLogFileHandler('DoUserCombine');
		$log = Log::Init($logHandler, 15);
		$retval = -1;
		if (request()->isAjax()) {
			$uid = request()->post('uid', 0);
			$target_uid = request()->post('target_uid', 0);
			$ps_code = request()->post('ps_code', '');
			if($ps_code != 'tmartgood'){
				return AjaxReturn(-3);	//操作码不正确
			}

			$member = new MemberService();
			$current_user = $member->getUserInfoByUid($uid);		//将信息合并到这个账户
			$old_user = $member->getUserInfoByUid($target_uid);	//目前账户，将其合并以后会被删除

			if(!empty($current_user) && !empty($old_user )){
				
				$log->INFO('old_uid['. $target_uid .']=> uid['. $uid .']');

				$user = new UserModel();
				$data = array();
				if(empty($current_user['user_name']) && !empty($old_user['user_name'])){
					$data['user_name'] = $old_user['user_name'];
					$data['user_password'] = $old_user['user_password'];
				}
				//判断手机
				if(empty($current_user['user_tel']) && !empty($old_user['user_tel'])){
					$data['user_tel'] = $old_user['user_tel'];
				}

				//判断wx_info
				if(empty($current_user['wx_info']) && !empty($old_user['wx_info'])){
					$data['wx_info'] = $old_user['wx_info'];
				}

				//wx_openid
				if(empty($current_user['wx_openid']) && !empty($old_user['wx_openid'])){
					$data['wx_openid'] = $old_user['wx_openid'];
				}

				//wx_unionid
				if(empty($current_user['wx_unionid']) && !empty($old_user['wx_unionid'])){
					$data['wx_unionid'] = $old_user['wx_unionid'];
				}

				//wx_applet_openid
				if(empty($current_user['wx_applet_openid']) && !empty($old_user['wx_applet_openid'])){
					$data['wx_applet_openid'] = $old_user['wx_applet_openid'];
				}

				//ali_user_id
				if(empty($current_user['ali_user_id']) && !empty($old_user['ali_user_id'])){
					$data['ali_user_id'] = $old_user['ali_user_id'];
				}

				//ali_info
				if(empty($current_user['ali_info']) && !empty($old_user['ali_info'])){
					$data['ali_info'] = $old_user['ali_info'];
				}
				...
				try{
					...
					if(!empty($data)){
						...
						$log->INFO('更新的sys_user数据['. $uid .']：'. json_encode($data));
					}

					//2. 查询会员账户信息`ns_member_account` 进行金额和积分（累加）合并  point  balance  coin  member_cunsum   member_sum_point
					$account_service = new MemberAccount();
					$account = $account_service->getMemberAccountInfo($uid);
					$account_old = $account_service->getMemberAccountInfo($target_uid);
					$account_data = array(
						'point' => $account['point'] +$account_old['point'],
						'balance' => $account['balance'] +$account_old['balance'],
						'coin' => $account['coin'] +$account_old['coin'],
						'member_cunsum' => $account['member_cunsum'] +$account_old['member_cunsum'],
						'member_sum_point' => $account['member_sum_point'] +$account_old['member_sum_point'],
					);
					$member_account_model = new NsMemberAccountModel();
					$member_account_model->save($account_data,  [ 'uid' => $uid ]);		//累加积分信息
					$log->INFO('current account['. $uid .']：'. json_encode($account));
					$log->INFO('account_old['. $target_uid .']：'. json_encode($account_old));
					$log->INFO('累加积分余额['. $uid .']：'. json_encode($account_data));

					//3. 开始执行用户的其它操作
					$user->execute("UPDATE ns_coupon SET uid = ".$uid." WHERE uid = ".$target_uid);
					$user->execute("UPDATE ns_member_account_records SET uid = ".$uid." WHERE uid = ".$target_uid);
					$user->execute("UPDATE ns_member_balance_withdraw SET uid = ".$uid." WHERE uid = ".$target_uid);
					$user->execute("UPDATE ns_member_behavior_records SET uid = ".$uid." WHERE uid = ".$target_uid);
					$user->execute("UPDATE ns_member_recharge SET uid = ".$uid." WHERE uid = ".$target_uid);
					$user->execute("UPDATE sys_user_log SET uid = ".$uid." WHERE uid = ".$target_uid);
					$user->execute("UPDATE ns_order SET buyer_id = ".$uid." WHERE buyer_id = ".$target_uid);
					$user->execute("UPDATE ns_order_action SET uid = ".$uid." WHERE uid = ".$target_uid);
					$user->execute("UPDATE ns_order_goods SET buyer_id = ".$uid." WHERE buyer_id = ".$target_uid);
					$user->execute("UPDATE ns_order_goods_express SET uid = ".$uid." WHERE uid = ".$target_uid);
					$user->execute("UPDATE ns_order_refund_account_records SET buyer_id = ".$uid." WHERE buyer_id = ".$target_uid);
					$log->INFO('执行update用户相关数据成功');
		
					//4. 删除已经合并的用户
					$member->deleteMember($target_uid);		
					$log->INFO('deleteMember['. $target_uid .']');

					//5. 写入合并成功日志
					$member->addUserLog($this->uid, 1, '用户', '合并用户', '将用户ID:['.$target_uid.'] => [.$uid.] 用户昵称：' .$current_user['nick_name']);
		
					// 提交事务
					$user->commit();   
					$log->INFO('提交事务');
					return AjaxReturn(1);	//返回成功
				} catch (\Exception $e) {
	 ...
	 * 列出可以匹配的合并用户
	 */
	public function autoMatchUser()
	{
		$member = new MemberService();
		if (request()->isAjax()) {
			$uid = request()->post('uid', 0);
			$auto_match = request()->post('autoget', 0);
			$curruser = $member->getUserInfoByUid($uid);
			if(empty($curruser)){
				return [];
			}
			...
			$condition['uid'] = array('<>',$uid);	//不等于当前合并用户

			$isFind = 0;
			if($auto_match == 1){
				//通过小程序或者公众号的openid 自动关联
				$orsql = '';
				if(!empty($curruser['wx_openid'])){
					$orsql = $orsql. "wx_openid='".$curruser['wx_openid']."'";
					$isFind = 1;
				}
				if(!empty($curruser['wx_applet_openid'])){
					$orsql = (empty($orsql)? "" :$orsql." OR "). "wx_applet_openid='".$curruser['wx_applet_openid'].
						"' OR wx_unionid='".$curruser['wx_applet_openid']."'";
					$isFind = 1;
				}
				if(!empty($curruser['wx_unionid'])){
					$orsql = (empty($orsql)? "" :$orsql." OR "). "wx_applet_openid='".$curruser['wx_unionid'].
						"' OR wx_unionid='".$curruser['wx_unionid']."'";
					$isFind = 1;
				}

				if($isFind == 1){
					$condition[''] = array('EXP',$orsql);
				}
			...
			$curarr[0] = $curruser;
			if($isFind == 1){
				$list = $member->getSysUserList($condition);
				//$sss = Db::getLastSql();
				$list = array_merge($curarr,$list );
			}else{
				return $curarr;
			}
			return $list;
