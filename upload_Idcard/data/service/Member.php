 ...
 * Member.php
 *
 * IBOMART商城系统 - 全球电商解决方案
 * =========================================================
 * Copy right 2015-2025 IBOMART.COM, 保留所有权利。
 * ----------------------------------------------
 * 官方网址: http://www.ibomart.com
...
class Member extends User
{
	 ...
	 *注册会员
	 */
	public function registerMember($user_name, $password, $email, $mobile, $user_qq_id, $qq_info, $wx_openid, $wx_info, $wx_unionid = "", $ali_user_id = "", $ali_info = "",$applet_openid = "")
	{
		if (!empty($user_qq_id) || !empty($wx_openid) || !empty($wx_unionid) ||!empty($ali_user_id) || !empty($applet_openid) ) {
			$is_false = true;
		} else {
			//数据验证
			if (trim($user_name) != "") {
		...
		$res = parent::add($user_name, $password, $email, $mobile, 0, $user_qq_id, $qq_info, $wx_openid, $wx_info, $wx_unionid, 1, 0 , $ali_user_id, $ali_info,$applet_openid);	//新增0 , $ali_user_id, $ali_info
		if ($res > 0) {
			// 获取默认会员等级id
...
//			runhook('Notify', 'registAfter', $params);
			...
			if (!empty($user_name)) {
				...
				$this->wchatUnionLogin($wx_unionid);		//unionid 登录
			}elseif (!empty($applet_openid)) {
				$this->wchatAppletLogin($applet_openid);		//小程序openid 登录 20210302
			} elseif (!empty($wx_openid)) {
				$this->wchatLogin($wx_openid);
			} elseif (!empty($ali_user_id)) {			//支付宝登录 新增内容
				$this->aliUserLogin($ali_user_id);		
			} 
...
//	         $api_secure = $config->getApiSecureConfig();
	 ...
	 * 获取会员登录后的初始化信息
	 */
	public function getMemberLoginInfoByUid($uid)
	{
		$this->initApp();
		if (!empty($uid)) {
			//生成token
...
//	         $api_secure = $config->getApiSecureConfig();
	...
	public function getMemberDefaultAddress($uid)
	{
		...
		if (!empty($data)) {
			$address = new Address();
			$address_info = $address->getAddress($data['province'], $data['city'], $data['district']);
			$data['address_info'] = $data['address']. ', ' .$address_info ;
		}
		return $data;
	}
	...
	public function addMemberAccount($shop_id, $type, $uid, $num, $text)
	{
		...
		if($retval > 0){
			...
			//记录日志
			$this->addUserLog($this->uid, 1, '商家调整', '调整'.$typeName, '被调整的用户id:' . $uid.'  类型:'.$typeName. '【 额度:'.$num.'】');
...
//				hook('memberWithdrawAuditAgree', [ 'id' => $id, 'type' => 'balance' ]);
	...
	public function memberGetCoupon($uid, $coupon_type_id, $get_type)
	{
		...
		$coupon_type_info = $coupon_type->getInfo([
			'coupon_type_id' => $coupon_type_id
		], 'max_fetch, start_time, end_time, term_of_validity_type,status,is_end,is_show');
		if (empty($coupon_type_info)) {
			return 0;
		}

		//完善领券权限认证
		if($coupon_type_info['status'] != 0){
			//优惠券状态（0、正常 1、已发完 2、已关闭）
			return -1;
		}

		if($get_type == 2 || $get_type == 3){
			if($coupon_type_info['is_show'] != 1){
				//是否允许首页显示0不显示1显示
				return -1;
			}
		}
	...
	public function getIdCardInfo(){
		$idCard = new UserModel();
		$condition['nc.uid'] = $this->uid;
		$idcard_info = $idCard->alias('nc')
			->field('nc.uid, nc.idcard_front, nc.idcard_back')->where($condition)->select();
		return $idcard_info;
	}
